...
This code is a Java implementation of the game of Checkers. The class GameSearch contains various methods that
allow for executing moves, finding valid moves, checking for jumps, and performing a minimax search to find the best move.

The executeMove method takes an old move and an old board as input, and returns a new board where the old move has been executed. If the old move is a jump, then the executeJump method is called instead. This method removes the captured checker (if any) and moves the checker to its destination. If multiple jumps are possible, then each possible jump is executed, and the resulting boards are compared to find the best one.

The executeUserJump method is similar to executeJump, but it does not continue with multiple jumps, since this
method is called when the user makes a move.

The moveChecker method moves a checker to its destination on the board. If the checker reaches the opponent's king row,
then it is promoted to a king.

The removeCapturedChecker method removes a checker that has been captured during a jump.

The findAllValidMoves method returns a list of all valid moves for a player on a board. If a jump is possible,
then only jumps are returned, since jumps are mandatory in Checkers.

The existJump method returns true if there is a jump among the valid moves, and false otherwise.

The removeNormalMoves method removes all normal moves from a list of moves if a jump is present,
since a jump is mandatory if it is possible.

The minimaxAB method performs a minimax search with alpha-beta pruning to find the best move for a player.
It builds and evaluates the game tree concurrently, rather than searching an already built game tree. */
